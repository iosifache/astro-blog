...
description: Software development principles, seasoned with a bit of Python
tags: ["development", "principles", "python"]
---
...
- Use code to document the code.
  - `employee.isEligibleForFullBenefits()`, not `if (employee.hasFlag(HOURLY) && employee.age > 65) // check if the employee is eligible for full benefits`
- Use good comments:
...
- Do one thing.
  - `savePageToFile` calls multiple functions to: download page and write the content to a file. It does not execute this whole functionality on its own.
- Only one level of abstraction per function
  - A function `getLinksFromHTMLPage` downloads the page and calls another function to search the effective text with a RegEx (which is at another abstraction level).
- Do only what's expected.
  - A `checkPassword` function will not initialize a session.
...
- **Law of Demeter**: A method should not know about the implementation of an object it manipulates.
  - `user.profilePicture.badge`, not `user.getProfilePicture().getBadge()`.
